...
def favorite_activities():
    ...
    fav=[]
    for day in days:
       favorite = input(f'What is your favorite thing to do on {day} ? ')
       fav.append(favorite)
    ...
    for i in range (len(days)):
        print(f'On {days[i]}s, your favorite activity is to {fav[i]}.')
...
def temp_by_day():
    ...
    for day in days:
      temp = int(input(f"What is the temperature today {day}: "))
      if temp < 50:
...
#     # Spring has sprung!
    current_temp = int(input("What is the temperature outside? "))
    while current_temp < 65:
